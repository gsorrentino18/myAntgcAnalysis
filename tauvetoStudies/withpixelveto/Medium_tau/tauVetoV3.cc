		...
		if(isMC) {
		   ...
		   break;
                ...
                for(Int_t i = 0; i < _nTau; i++){
                     ...
                     if(!getBit(tmptauIDbitsDeepTau2017v2p1, 4)) continue;
                     if(!getBit(tmptauIDbitsDeepTau2017v2p1, 12)) continue;
                     if(!getBit(tmptauIDbitsDeepTau2017v2p1, 18)) continue;
               ...
               for(UShort_t iPho=0; iPho<_nPho; iPho++){
                  ...
                  if (fabs(_phoEta[iPho]) > 2.5) continue;
                  if (_phoCalibEt[iPho] < 20.) continue;

                  if (_phoCalibEt[iPho] < highestPt) {
                     noID_ = 1;
                     Char_t tmpphoIDbit = _phoIDbit[iPho];
                     if(getBit(tmpphoIDbit, 0)) {loose_=1; } //photon ID
                     if(getBit(tmpphoIDbit, 1)) {medium_=1; }
                     if(getBit(tmpphoIDbit, 2)) {tight_=1; }
                     if(getBit(tmpphoIDbit, 3)) {WP80_=1; }
                     if(getBit(tmpphoIDbit, 4)) {WP90_=1; } 

                     Short_t highetsPtGenEleIndex = -9999;    //// sometimes there can be duplicate copies of gen particles
                     Short_t minimumdRGenPhoIndex = -9999;
                     Float_t highestPtEle = -999.;
                     Float_t maxdRPho = 999.;

                     for(UShort_t iGenP=0; iGenP<_nMC; iGenP++){

                        if(_mcStatus[iGenP] != 1) continue;     // final state (stable)

                        if(std::abs(_mcPID[iGenP]) == 11) { //only electrons
                          UShort_t iGenPStFl = _mcStatusFlag[iGenP];
                          if(getBit(iGenPStFl,1)) {       // isPromptFinalState()

                            if (std::abs(_mcMomPID[iGenP]) == 24) { isFromW_ = 1;} //is from W

                            if(_mcPt[iGenP] > highestPtEle){
                              highetsPtGenEleIndex = iGenP;
                              highestPtEle = _mcPt[iGenP];
                            }
                          }
                        }

                        if(std::abs(_mcPID[iGenP]) == 22) {

                          if (_mcPromptStatusType[iGenP] ==0) {SUSYprompt_ = 1;}
                            Float_t dRtemp = deltaR(_mcEta[iGenP], _mcPhi[iGenP], _phoEta[iPho], _phoPhi[iPho]);
                            if (dRtemp < maxdRPho) {
                              minimumdRGenPhoIndex = iGenP;
                              maxdRPho = dRtemp;
                            }
                        }
                     }

                     if (highetsPtGenEleIndex > 0) {
                       pdgID_ = std::abs(_mcPID[highetsPtGenEleIndex]);
                       deltaRele_ = deltaR(_mcEta[highetsPtGenEleIndex], _mcPhi[highetsPtGenEleIndex], _phoEta[iPho], _phoPhi[iPho]);   
                     }
                     if (minimumdRGenPhoIndex > 0) {
                       pdgID_ = std::abs(_mcPID[minimumdRGenPhoIndex]);
                       deltaRpho_ = deltaR(_mcEta[minimumdRGenPhoIndex], _mcPhi[minimumdRGenPhoIndex], _phoEta[iPho], _phoPhi[iPho]); 
                     }
                  }
...
void tauVeto::initEventType(eventType & evType, std::string typeName, std::string typeTitle){
	...
	evType.tree->Branch("minDeltaRPhoJet30", &minDeltaRPhoJet30_);
        ...
        evType.tree->Branch("noID", &noID_);
        evType.tree->Branch("loose", &loose_);
        evType.tree->Branch("medium", &medium_);
        evType.tree->Branch("tight", &tight_);
        evType.tree->Branch("WP80", &WP80_);
        evType.tree->Branch("WP90", &WP90_);
        evType.tree->Branch("SUSYprompt", &SUSYprompt_);
        evType.tree->Branch("isFromW", &isFromW_);
        evType.tree->Branch("deltaRele", &deltaRele_);
        evType.tree->Branch("deltaRpho", &deltaRpho_);
        evType.tree->Branch("pdgID", &pdgID_);
