...
void Validation_gjets(){
   ...
   TString path = "/local/cms/user/gsorrent/antgc_analysis/BDTValidation/UL17/EBNtuples_gjets_correct/merged/";
   TString savePath = "/local/cms/user/gsorrent/antgc_analysis/BDTValidation/UL17/EBPlots_gjets/";
   if (isEE) {
      path = "/local/cms/user/gsorrent/antgc_analysis/BDTValidation/UL17/EENtuples_gjets/merged/";
      savePath = "/local/cms/user/gsorrent/antgc_analysis/BDTValidation/UL17/EEPlots_gjets/";
   }

   if (is2018) {
      path = "/local/cms/user/gsorrent/antgc_analysis/BDTValidation/UL18/EBNtuples_gjets/merged/";
      savePath = "/local/cms/user/gsorrent/antgc_analysis/BDTValidation/UL18/EBPlots_gjets/";
      if (isEE) {
        path = "/local/cms/user/gsorrent/antgc_analysis/BDTValidation/UL18/EENtuples_gjets/merged/";
        savePath = "/local/cms/user/gsorrent/antgc_analysis/BDTValidation/UL18/EEPlots_gjets/";
      }
   }
   ...
   for (Int_t i=0; i<nfile_eff;i++) {
     ...
     tree[i]->SetBranchAddress("jetphoDeltaPhi", &ph_jet_dphi);
     tree[i]->SetBranchAddress("nPhoCand", &ph_ncand);
     //tree[i]->SetBranchAddress("phoESEffSigmaRR", &ph_ESEffSigmaRR);
   }

   const Int_t nhist=22;
   ...
   for (Int_t f=0; f<nfile_eff;f++) {
        ...
        hist[f][21] = new TH1F("ph_ESEffSigmaRR"+sample[f],"ph_ESEffSigmaRR"+sample[f],30,-0.1,15);
   }
   ...
   for (Int_t fil=0; fil<nfile_eff; fil++) {
    ...
    Double_t sumGenWeight = cutFlowGenWeight->GetBinContent(1);
     ...
     for (Int_t i=0; i<nentries[fil]; i++) {
        ...
        if (!isEE) {
          if (ph_TrkIsoCorr > 2.5) continue;
          if (ph_PFHCALClusIsoCorr > 7) continue;
        }

        //std::cout << ph_ncand << "  ";  
        if (ph_jet_dphi > 2.9) continue;
        if (ph_ncand != 1) continue;
        ...
        if (fil==0) {
          ...
          //hist[fil][21]->Fill(ph_ESEffSigmaRR);
        } else if (fil!=0) {
   ...
   for (int i=0; i<nhist; i++) {
      ...
      //hist[4][i]->Draw("hist");
      //c->SaveAs(savePath+title[i]+"_newfile.png");
      ...
      hist[0][i]->Draw("ep");
      hs[i]->Draw("hist, SAME");
      //h_allScaled[i]->SetHistLineColor(15);
      //h_allScaled[i]->SetHistLineStyle(2);
      ...
      //std::cout << "bin and error MC: " << h_allScaled[0]->GetBinContent(3) << "  "<<h_allScaled[0]->GetBinError(3) << std::endl;
      //std::cout << "bin and error data: " << hist[0][0]->GetBinContent(3) << "  "<<hist[0][0]->GetBinError(3) << std::endl;
      //std::cout << "bin and error ratio: " << h_ratio[0]->GetBinContent(3) << "  "<<h_ratio[0]->GetBinError(3) << std::endl;      

      TLegend* leg = new TLegend();
      leg = new TLegend(0.68, 0.5, 0.9, 0.89);
      leg->SetBorderSize(0);
      ...
      if (is2017) {
         ...
         leg->AddEntry(hist[22][i], "WW, WZ, ZZ", "f");
         //leg->AddEntry(hist[23][i], "WZ", "f");
         //leg->AddEntry(hist[24][i], "ZZ", "f");
         leg->AddEntry(hist[25][i], "W+jets", "f");
      }
   ...
   if (do_cumulative) { //check normalization
