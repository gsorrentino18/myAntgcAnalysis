...
for file in "${FILES[@]}"; do
	...
	normTermLine=$(tac $file | grep -m 1 "Normal termination")
        #killLine=$(tac $file | grep -m 1 "Job was evicted")
        killLine=$(tac $file | grep -m 1 "Job was not checkpointed")
	killLine=$(tac $file | grep -m 1 "Job was aborted by the user")
        killLine=$(tac $file | grep -m 1 "condor_exec.exe: line 8: root: command not found")
	...
	if ( [ ! -z "$normTermLine" ] && [ ! -z "$killLine" ] ); then
		...
		resubmit $file
                ...
                killLineN=$(tac $file | grep -n -m 1 "condor_exec.exe: line 8: root: command not found" | cut -d : -f 1)
